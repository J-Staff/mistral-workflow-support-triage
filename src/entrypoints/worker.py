...
def discover_workflows() -> list[type]:
    """Scan the `workflows` package recursively and return all workflow classes.

    Uses ``walk_packages`` so sub-packages (e.g. ``workflows.support_triage``)
    are descended into. ``iter_modules`` would stop at the first level.
    De-duplicates by class identity in case a workflow is re-exported.
    """
